...
function calculateWeightedMetrics(original, modified, language) {
  ...
  const baseline = Math.max(originalWeight, 1);
  const percentage = Number(((changes / baseline) * 100).toFixed(2));
...
export function countChanges(original, modified, language) {
  return calculateWeightedMetrics(original, modified, language).changes;
}

export function calculateChangePercentage(original, modified, language) {
  return calculateWeightedMetrics(original, modified, language).percentage;
}
